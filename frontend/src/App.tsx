...
function App() {
  ...
  useEffect(() => {
    if (currentType) {
      fetch(`http://localhost:3001/records/${dateStr}/${currentType}`)
        .then(r => r.json())
        .then(data => {
                    ...
                    {Array.from({ length: 7 }).map((_, col) => (
                      <td key={col} className="p-2">
                        <input
                          ...
                          className="w-full bg-gray-950 border border-gray-600 rounded-xl px-4 py-3 text-base outline-none focus:border-blue-500"
